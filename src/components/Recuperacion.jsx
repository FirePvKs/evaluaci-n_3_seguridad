...
const fasesDRP = [
  {
    ...
    disparadores: ['Alerta WAF por payload de SQLi o CMDi', 'Anomalía de volumen de queries en BD de clientes', 'Alerta SIEM por proceso hijo inesperado del servidor web'],
  },
  {
    fase: 'FASE 1',
    color: '#c0392b',
    ...
    acciones: ['Colocar el Portal de Clientes en modo mantenimiento', 'Revocar todos los tokens de sesión activos de clientes', 'Desconectar el servidor comprometido de la red de DMZ', 'Revocar credenciales de conexión actuales a la base de datos'],
  },
  {
    fase: 'FASE 2',
    color: '#e67e22',
    ...
    acciones: ['Auditoría forense del sistema de archivos del servidor comprometido', 'Identificar qué datos financieros fueron accedidos o alterados', 'Parchado de la vulnerabilidad explotada en el código fuente', 'Eliminación de web shells o malware instalado por el atacante'],
  },
  {
    fase: 'FASE 3',
    color: '#2980b9',
          ...
          {mejoras.map((m) => (
            <div key={m.letra} style={{ border: '1px solid var(--border)', borderRadius: '10px', padding: '16px', background: 'var(--accent-bg)' }}>
              ...
              <p style={{ fontSize: '13px', margin: '0 0 6px' }}>
                <strong>Acción:</strong> {m.accion}
              </p>
              <p style={{ fontSize: '13px', margin: 0 }}>
                <strong>Propósito:</strong> {m.proposito}
              </p>
              {m.capas && (
            ...
            <div key={f.fase}>
              <div style={{ border: `2px solid ${f.color}`, borderRadius: '10px', padding: '16px', background: 'var(--accent-bg)' }}>
                <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '8px', flexWrap: 'wrap' }}>
                  ...
                  <span style={{ background: f.color, color: '#fff', borderRadius: '4px', padding: '2px 10px', fontSize: '12px', fontWeight: 700 }}>
                    {f.fase}
                  </span>
              ...
              {idx < fasesDRP.length - 1 && (
                <div style={{ display: 'flex', justifyContent: 'center', padding: '4px 0' }}>
                  <span style={{ fontSize: '20px', color: 'var(--text)', opacity: 0.5 }}>↓</span>
                </div>
              )}
            </div>
          ))}
      ...
      <div className="card">
        ...
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: '200px', padding: '12px', background: 'var(--code-bg)', borderRadius: '8px', borderLeft: '3px solid var(--accent)' }}>
            <p style={{ margin: '0 0 4px', fontWeight: 700, fontSize: '13px', color: 'var(--text-h)' }}>⏱ Respaldo incremental</p>
          ...
          <div style={{ flex: 1, minWidth: '200px', padding: '12px', background: 'var(--code-bg)', borderRadius: '8px', borderLeft: '3px solid var(--accent)' }}>
            <p style={{ margin: '0 0 4px', fontWeight: 700, fontSize: '13px', color: 'var(--text-h)' }}>📦 Respaldo completo (Full)</p>
